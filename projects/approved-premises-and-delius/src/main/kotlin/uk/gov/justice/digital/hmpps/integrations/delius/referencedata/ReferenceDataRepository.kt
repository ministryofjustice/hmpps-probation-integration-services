...
fun ReferenceDataRepository.acceptedDeferredAdmission() =
    findByCodeAndDatasetCode("AD", DatasetCode.REFERRAL_DECISION)
        ?: throw NotFoundException("ReferralDecision", "code", "AD")

fun ReferenceDataRepository.apReferralSource() =
    findByCodeAndDatasetCode("AP", DatasetCode.SOURCE_TYPE)
        ?: throw NotFoundException("SourceType", "code", "AP")

fun ReferenceDataRepository.ynUnknown() =
    findByCodeAndDatasetCode("D", DatasetCode.YES_NO)
        ?: throw NotFoundException("YesNo", "code", "D")

fun ReferenceDataRepository.unknownRisk() =
    findByCodeAndDatasetCode("K", DatasetCode.RISK_OF_HARM)
        ?: throw NotFoundException("RiskOfHarm", "code", "K")

fun ReferenceDataRepository.referralCompleted() =
    findByCodeAndDatasetCode("APRC", DatasetCode.NSI_OUTCOME)
