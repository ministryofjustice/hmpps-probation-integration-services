...
class Document(
    ...
    companion object {
        const val BREACH_NOTICE_URN_PREFIX: String = "urn:hmpps:breach-notice:"

        fun breachNoticeUrn(uuid: UUID): String =
            "$BREACH_NOTICE_URN_PREFIX$uuid"
    }
}
...
interface DocumentRepository : JpaRepository<Document, Long> {
    ...
    @Query(
        """
        select document.external_reference
        from document
            -- the following joins are to get the event_id from the related entities, for event-level documents
        left join event on document.table_name = 'EVENT' and document.primary_key_id = event.event_id
        left join court_report
        ...
        left join disposal upw_appointment_disposal on upw_appointment_disposal.disposal_id = upw_details.disposal_id
        left join contact
                  on (document.table_name = 'CONTACT' and document.primary_key_id = contact.contact_id)
                  or (document.table_name = 'UPW_APPOINTMENT' and upw_appointment.contact_id = contact.contact_id)
        left join nsi on document.table_name = 'NSI' and document.primary_key_id = nsi.nsi_id
        where document.external_reference like 'urn:hmpps:breach-notice:%'
          and coalesce(event.event_id,
                       ...
                       upw_appointment_disposal.event_id,
                       contact.event_id,
                       nsi.event_id) = :eventId
        """,
        nativeQuery = true
    )
    fun findBreachNoticeUrnsByEventId(eventId: Long): List<String>
}
