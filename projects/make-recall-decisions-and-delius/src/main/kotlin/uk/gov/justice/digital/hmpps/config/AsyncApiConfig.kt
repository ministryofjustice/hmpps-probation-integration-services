...
class AsyncApiConfig {
    ...
    fun asyncApiExtension(asyncApiCommonInfo: AsyncApiExtension) = AsyncApiExtension.builder(order = 1) {
        info.title("make-recall-decisions-and-delius")
        info.description("Reflect Activity in the Consider a Recall Service in Delius")
        servers {
            server("dev") {
        ...
        externalDocs {
            url("https://ministryofjustice.github.io/hmpps-probation-integration-services/tech-docs/projects/make-recall-decisions-and-delius/")
        }
    }
}
