...
internal class CreateCaseNoteIntTests @Autowired constructor(
    ...
    fun `create contact for RP9 when author does not exist`() {
        val crn = DEFAULT.crn
        mockMvc.post("/nomis-case-note/$crn") {
    ...
    fun `return bad request when author prison code is not found`() {
        val crn = DEFAULT.crn
        val res = mockMvc.post("/nomis-case-note/$crn") {
            contentType = MediaType.APPLICATION_JSON
            content = """
                        {
                            "type": "PRE_RELEASE_REPORT",
    ...
    fun `return not found when crn is not found`() {
        mockMvc.post("/nomis-case-note/X123456") {
            contentType = MediaType.APPLICATION_JSON
            content =
                """
                    {
