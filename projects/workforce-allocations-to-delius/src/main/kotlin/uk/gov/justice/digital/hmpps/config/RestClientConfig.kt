...
class RestClientConfig(private val clientManager: OAuth2AuthorizedClientManager) {
    ...
    @Bean
    fun workforceAllocationsClient(
        restClientBuilder: RestClient.Builder
    ): WorkforceAllocationsClient {
        val exchange = RestClientAdapter.create(
            restClientBuilder
                .requestInterceptor(HmppsAuthInterceptor(clientManager, "workforce-allocations"))
                .build()
        )
        return HttpServiceProxyFactory.builderFor(exchange).build()
            .createClient(WorkforceAllocationsClient::class.java)
    }
}
