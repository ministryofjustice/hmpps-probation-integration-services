...
class ImpactIntegrationTest @Autowired constructor(
    private val mockMvc: MockMvc
) {

    @Test
    fun `get impact unauthorised`() {
        mockMvc.get("/allocation-demand/impact?crn=N542873&staff=N012DT")
            .andExpect { status { isUnauthorized() } }
    }

    @Test
    fun `get impact no matching crn`() {
        mockMvc.get("/allocation-demand/impact?crn=N542873&staff=N012DT") { withToken() }
            .andExpect { status { isNotFound() } }
    }

    @Test
    fun `get impact no matching staff`() {
        mockMvc.get("/allocation-demand/impact?crn=${PersonGenerator.DEFAULT.crn}&staff=N01DTT1") { withToken() }
            .andExpect { status { isNotFound() } }
    }

    @Test
    fun `get impact no crn provided`() {
        mockMvc.get("/allocation-demand/impact?staff=N01DTT1") { withToken() }
            .andExpect { status { isBadRequest() } }
    }

    @Test
    fun `get impact no staff code provided`() {
        mockMvc.get("/allocation-demand/impact?crn=${PersonGenerator.DEFAULT.crn}") { withToken() }
            .andExpect { status { isBadRequest() } }
    }
    ...
    fun `get impact returns person and staff`() {
        ...
        val impact =
            mockMvc.get("/allocation-demand/impact?crn=${PersonGenerator.DEFAULT.crn}&staff=${staff.code}") {
                withToken()
            }
