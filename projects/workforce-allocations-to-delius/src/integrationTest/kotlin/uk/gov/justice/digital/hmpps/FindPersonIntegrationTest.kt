...
class FindPersonIntegrationTest @Autowired constructor(
    private val mockMvc: MockMvc
) {

    @Test
    fun `get person unauthorised`() {
        mockMvc.get("/person/X123456?type=CRN")
    ...
    fun `get person no matching crn`(value: String, type: String) {
        mockMvc.get("/person/$value?type=$type") { withToken() }
    ...
    fun `find reallocation details`() {
        ...
        mockMvc.get("/person/${person.crn}/reallocation-details") { withToken() }
            .andExpect { status { is2xxSuccessful() } }
            .andExpectJson(
                ReallocationDetails(
