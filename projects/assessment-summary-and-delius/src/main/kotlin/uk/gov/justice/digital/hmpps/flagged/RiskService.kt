...
/**
 * This RiskService implements the old registration recording logic, and is required until the "assessment-summary_update-risk-registrations-in-place" feature is enabled in all environments.
 *
 * Once the "assessment-summary_update-risk-registrations-in-place" feature has been enabled, the entire "flagged" package can be deleted.
 */
@Service("FlaggedRiskService")
class RiskService(
    private val registrationRepository: FlaggedRegistrationRepository,
    ...
    private fun recordOtherRisks(
        ...
        RiskType.entries.flatMap { riskType ->
            ...
            val existingLevel = maxByCode(registrationsToRemove.mapNotNull { it.level?.code })
    ...
    private fun MutableList<FlaggedRegistration>.addRegistration(
        person: Person,
        type: RegisterType,
    ...
    private fun Person.removeRegistration(
        registration: FlaggedRegistration,
    ...
    private fun FlaggedRegistration.reviewContact(person: Person, notes: String = notes()) =
    ...
    fun FlaggedRegistration.notes(): String = reviewNotes(type, nextReviewDate)
    ...
    fun FlaggedRegistration.deRegEvent(crn: String): HmppsDomainEvent = HmppsDomainEvent(
    ...
    ).also { domainEventService.publish(it) }

    fun FlaggedRegistration.regEvent(crn: String): HmppsDomainEvent = HmppsDomainEvent(
