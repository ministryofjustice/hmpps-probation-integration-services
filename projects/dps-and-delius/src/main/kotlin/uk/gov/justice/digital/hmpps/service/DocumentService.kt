...
class DocumentService(
    ...
    fun downloadDocument(id: String, username: String): ResponseEntity<StreamingResponseBody> {
        ...
        val person = personRepository.findById(document.personId).orElseThrow { NotFoundException("Person", "id", document.personId) }
        checkForLao(person.crn, username)
        return alfrescoClient.streamDocument(id, document.name)
    }

    @Transactional
    fun getDocumentsForCase(nomisId: String, username: String) = personRepository.findByNomisId(nomisId)?.let { person ->
        checkForLao(person.crn, username)
