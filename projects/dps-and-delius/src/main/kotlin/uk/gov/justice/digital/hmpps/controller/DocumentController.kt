...
class DocumentController(private val documentService: DocumentService) {
    ...
    fun getDocuments(@PathVariable nomisId: String, authentication: Authentication) = documentService.getDocumentsForCase(nomisId, authentication.name)

    @GetMapping(value = ["/document/{id}"])
    @Operation(summary = "Download document content")
    fun downloadDocument(@PathVariable id: String, authentication: Authentication) = documentService.downloadDocument(id, authentication.name)
}
