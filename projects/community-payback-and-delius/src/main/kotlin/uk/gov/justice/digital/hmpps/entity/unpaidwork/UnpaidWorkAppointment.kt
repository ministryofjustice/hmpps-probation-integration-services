...
interface UnpaidWorkAppointmentRepository : JpaRepository<UnpaidWorkAppointment, Long> {
    ...
    @Query(
        ...
        left join (
            ...
            where trunc(cast(appointment_date as date)) + (cast(end_time as date) - trunc(cast(end_time as date))) between current_date - 45 and current_date
    ...
    @Query(
        ...
        where (:crn is null or a.person.crn = :crn)
          ...
          and (
            (:noOutcomeOnly = true and (o is null or o.code in :filteredOutcomeCodes)) or
            (:noOutcomeOnly = false and (:filteredOutcomeCodes is null or o.code in :filteredOutcomeCodes))
          )
        """
    )
