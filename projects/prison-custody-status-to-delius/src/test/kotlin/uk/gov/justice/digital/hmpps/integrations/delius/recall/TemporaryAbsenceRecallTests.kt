...
class TemporaryAbsenceRecallTests : RecallServiceTestBase() {
    ...
    fun `temporary absence with current status RoTL`() {
        ...
        whenever(recallReasonRepository.findByCode(RecallReason.Code.END_OF_TEMPORARY_LICENCE.value))
        ...
        assertThat(
            ...
            equalTo("Reason for Recall: description of EOTL$EOTL_RECALL_CONTACT_NOTES")
        )
    ...
    @Test
    fun `transferred with current status RoTL`() {
        val person = PersonGenerator.generate("A12345T")
        val event = EventGenerator.previouslyReleasedEvent(
            person,
            InstitutionGenerator.DEFAULT,
            custodialStatusCode = CustodialStatusCode.CUSTODY_ROTL
        ...
        whenever(recallReasonRepository.findByCode(RecallReason.Code.END_OF_TEMPORARY_LICENCE.value)).thenReturn(recallReason)
