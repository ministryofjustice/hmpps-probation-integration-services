...
/*+ INLINE LEADING(fc) */
with
    filtered_caseload as (
        select c.offender_id, c.trust_provider_team_id
        from caseload c
    ...
    dt as (
        select trunc(sysdate) as today, sysdate as now_ from dual
    ),

    next_date as (
        select /*+ INDEX(c XIE18CONTACT) */
            fc.offender_id,
            min(c.contact_date) as next_date
        from filtered_caseload fc
        cross join dt
        join contact c
             on c.offender_id = fc.offender_id
        where c.soft_deleted = 0
          and (
            c.contact_date >  dt.today
                or (c.contact_date = dt.today and c.contact_start_time > dt.now_)
            )
          and c.contact_date <= dt.today + 1825
          and exists (
            select 1
            from r_contact_type ct
            where ct.contact_type_id = c.contact_type_id
...
-- 6) Bring the chosen NEXT contact details
    next_app as (
    ...
    sentence_base as (
        select e.offender_id,
               to_number(e.event_number) as event_no,
               d.disposal_id
        from filtered_caseload fc2
        join event e
             on e.offender_id   = fc2.offender_id
                 and e.active_flag   = 1
...
select
    ...
    (na.contact_date
        + (na.contact_start_time - trunc(na.contact_start_time))) as next_appointment_date_time,
    na.type_description                as next_appointment_type_description,

    pa.contact_id                      as prev_appointment_id,
    (pa.contact_date
        + (pa.contact_start_time - trunc(pa.contact_start_time))) as prev_appointment_date_time,
...
from filtered_caseload fc
join offender p
     on p.offender_id = fc.offender_id
join team t
     on t.team_id = fc.trust_provider_team_id
