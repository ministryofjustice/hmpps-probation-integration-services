...
fun uk.gov.justice.digital.hmpps.integrations.delius.risk.RiskFlag.toRiskFlag(
    ...
    removalHistory = deRegistrations.sortedByDescending { it.deRegistrationDate }
        .map { it.toRiskFlagRemoval(removalHistoryNoteId) }
)
...
fun DeRegistration.toRiskFlagRemoval(removalHistoryNoteId: Int? = null) = RiskFlagRemoval(
    ...
    riskRemovalNote = if (removalHistoryNoteId != null) formatNote(
        notes,
        false
    ).elementAtOrNull(removalHistoryNoteId) else null,
