    ...
    fun `cannot reschedule if appointment would clash`() {
        ...
        val clashing = sentenceAppointmentRepository.save(
            AppointmentGenerator.generateAppointment(
                ...
                ZonedDateTime.now().plusDays(2).plusMinutes(60)
            )
        )
        val request = rescheduleRequest(
            date = clashing.date,
            startTime = ZonedDateTime.now().plusDays(2).toLocalTime(),
            endTime = ZonedDateTime.now().plusDays(2).plusHours(1).toLocalTime(),
        )
