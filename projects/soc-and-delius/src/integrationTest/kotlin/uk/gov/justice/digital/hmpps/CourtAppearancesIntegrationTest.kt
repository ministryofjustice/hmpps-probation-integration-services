...
internal class CourtAppearancesIntegrationTest @Autowired constructor(
    ...
    fun `API call retuns a success response`() {
        ...
        mockMvc.get("/court-appearances/$crn") { withToken() }
    ...
    fun `API call retuns no results`() {
        val crn = CourtAppearanceGenerator.DEFAULT_PERSON.crn
        mockMvc.get("/court-appearances/$crn?fromDate=2099-12-12") { withToken() }
            .andExpect { status { is2xxSuccessful() } }
    ...
    fun `returns results for multiple CRNs`() {
        ...
        mockMvc.post("/court-appearances") {
            withToken()
            json = crns
        }
            .andExpect {
