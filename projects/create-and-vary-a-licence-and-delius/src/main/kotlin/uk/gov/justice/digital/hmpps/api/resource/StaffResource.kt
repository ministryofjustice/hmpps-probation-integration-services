...
class StaffResource(
    ...
    ): Staff = staffService.findStaff(username)

    @PreAuthorize("hasRole('CVL_CONTEXT')")
    @GetMapping("/{boroughCode}/pdu-head")
    fun findPDUHead(
        @PathVariable boroughCode: String,
    ): List<PDUHead> = staffService.findPDUHeads(boroughCode)

    @PreAuthorize("hasRole('CVL_CONTEXT')")
    @PostMapping
    fun findStaffForUsernames(
        @RequestBody usernames: List<String>,
    ): List<StaffName> = staffService.findStaffForUsernames(usernames)
}
