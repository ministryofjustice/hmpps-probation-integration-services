...
class DocumentEntity(
    ...
    companion object {
        const val COSSO_BREACH_NOTICE_URN_PREFIX: String = "urn:uk:gov:hmpps:breach:cosso-breach-notice:"

        fun cossoBreachNoticeUrn(uuid: UUID): String =
            "$COSSO_BREACH_NOTICE_URN_PREFIX$uuid"
    }
}
...
interface DocumentRepository : JpaRepository<DocumentEntity, Long> {
    ...
    @Query(
        ...
        left join contact on document.table_name = 'CONTACT' and document.primary_key_id = contact.contact_id
        left join nsi on document.table_name = 'NSI' and document.primary_key_id = nsi.nsi_id
        where document.external_reference like 'urn:uk:gov:hmpps:breach:cosso-breach-notice:%'
    ...
    fun findCossoBreachNoticeUrnsByEventId(eventId: Long): List<String>
}