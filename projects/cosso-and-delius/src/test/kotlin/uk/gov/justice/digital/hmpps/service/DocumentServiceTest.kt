...
class DocumentServiceTest {
    ...
    fun `updateParent sets document_linked correctly for supported tables`(tableName: String) {
        ...
        whenever(documentRepository.existsByTableNameAndPrimaryKeyIdAndIdNotAndSoftDeletedFalse(tableName, 123L, 1L)).thenReturn(true)
    ...
    fun `updateParent sets document_linked to N when no other documents exist`() {
        ...
        whenever(documentRepository.existsByTableNameAndPrimaryKeyIdAndIdNotAndSoftDeletedFalse("CONTACT", 123L, 1L)).thenReturn(false)
        whenever(entityManager.createNativeQuery(any())).thenReturn(query)
