...
internal class OffenceDetailsTest @Autowired constructor(private val mockMvc: MockMvc) {

    @Test
    fun `should return offence details`() {
        val offenceDetails = mockMvc.get("/offence-details/D006296") {
            withToken()
        }.andExpect {
    ...
    fun `should return HTTP not found when CRN does not exist`() {
        mockMvc.get("/offence-details/D000001") {
            withToken()
        }.andExpect {
            status { isNotFound() }
        }
