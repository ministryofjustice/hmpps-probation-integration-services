...
internal class AssessmentTimelineTest @Autowired constructor(private val mockMvc: MockMvc) {

    @Test
    fun `get latest layer 3 timeline assessment`() {
        val oasysTimelineAssessment = mockMvc.get("/latest-assessment/D006296") {
            withToken()
        }.andExpect {
    ...
    fun `should return HTTP not found when CRN does not exist`() {
        mockMvc.get("/latest-assessment/D000000") {
            withToken()
        }.andExpect {
            status { isNotFound() }
        }
    ...
    fun `should return HTTP not found when a layer 3 assessment does not exist`() {
        mockMvc.get("/latest-assessment/D000001") {
            withToken()
        }.andExpect {
            status { isNotFound() }
        }
