...
internal class HealthDetailsTest @Autowired constructor(private val mockMvc: MockMvc) {

    @Test
    fun `should return health details`() {
        val healthDetails = mockMvc.get("/health-details/D006296") {
            withToken()
        }.andExpect {
    ...
    fun `should return HTTP not found when CRN does not exist`() {
        mockMvc.get("/health-details/D000001") {
            withToken()
        }.andExpect {
            status { isNotFound() }
        }
