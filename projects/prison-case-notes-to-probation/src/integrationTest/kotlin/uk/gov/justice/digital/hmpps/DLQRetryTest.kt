...
class DLQRetryTest {
    ...
    fun `dlq messages are retried after a delay`() {
        ...
        jmsTemplate.convertSendAndWait(dlqName, CaseNoteMessageGenerator.NEW_TO_DELIUS)

        // then the message will be resent to the real queue immediately
        assertThat(jmsTemplate.getMessageCount(dlqName), equalTo(0))
        assertThat(jmsTemplate.getMessageCount(queueName), equalTo(1))
        ...
        val startTime = now()
        jmsTemplate.waitForQueueToBeEmpty(queueName)
