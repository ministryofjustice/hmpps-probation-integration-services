...
import uk.gov.justice.digital.hmpps.messaging.FEATURE_FLAG
...
internal class OpdIntegrationTest {
    ...
    fun `process opd assessment`() {
        whenever(featureFlags.enabled(FEATURE_FLAG)).thenReturn(true)
    ...
    fun `does not process opd assessment when feature flagged`() {
        whenever(featureFlags.enabled(FEATURE_FLAG)).thenReturn(false)
