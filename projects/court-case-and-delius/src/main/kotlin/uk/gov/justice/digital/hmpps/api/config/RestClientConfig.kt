...
class RestClientConfig(private val clientManager: OAuth2AuthorizedClientManager) {
    ...
    @Bean
    fun courtCaseClient(restClientBuilder: RestClient.Builder): CourtCaseClient {
        val exchange = RestClientAdapter.create(
            restClientBuilder
                .requestInterceptor(HmppsAuthInterceptor(clientManager, "court-case-and-delius"))
                .build()
        )
        return HttpServiceProxyFactory.builderFor(exchange).build()
            .createClient(CourtCaseClient::class.java)
    }
}
