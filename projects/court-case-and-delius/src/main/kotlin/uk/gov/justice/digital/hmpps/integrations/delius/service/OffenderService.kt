...
class OffenderService(
    ...
    private fun getOffences(event: Event): List<Offence> {
        ...
        val additionalOffences =
            additionalOffenceRepository.findByEvent(event).map {
    ...
    private fun getCourtReports(event: Event) =
        courtReportRepository.getAllByEvent(event).map {
            CourtReport(
                it.dateRequested,
                it.dateRequired,
                it.dateCompleted,
                ...
                author =
                    it.reportManagers.lastOrNull()?.let { m ->
                        ReportAuthor(
    ...
    private fun getPssRequirements(custodyId: Long?) =
        if (custodyId == null) {
            listOf()
        } else {
    ...
    private fun getBreaches(event: Event) =
        nsiRepository.findAllBreachNSIByEventId(event.id).map {
    ...
    private fun getConvictionDocuments(
        event: Event,
        documents: List<Document>,
