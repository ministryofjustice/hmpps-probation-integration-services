...
fun District.forManager() = uk.gov.justice.digital.hmpps.api.model.LocalDeliveryUnit(code, description)

fun Team.forManager() = uk.gov.justice.digital.hmpps.api.model.Team(code, description, district?.forManager())
...
fun Registration?.level(): Int =
    when (this?.level?.code) {
        "M1" -> 1
        "M2" -> 2
        "M3" -> 3
        else -> 0
