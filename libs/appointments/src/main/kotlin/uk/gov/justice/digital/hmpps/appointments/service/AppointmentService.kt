...
import uk.gov.justice.digital.hmpps.appointments.domain.contact.ContactOutcome
...
class AppointmentService(
    ...
    fun scheduleAppointment(request: CreateFutureAppointment, typeProvider: (String) -> ContactType): Contact =
    ...
    fun applyOutcome(request: ApplyOutcomeRequest, outcomeProvider: (String) -> ContactOutcome): Contact =
    ...
    fun recordAppointment(
        ...
        typeProvider: (String) -> ContactType,
        outcomeProvider: (String) -> ContactOutcome
    ): Contact = audit(BusinessInteractionCode.ADD_CONTACT, username = request.username) { audit ->
        val relatedTo = relatedToService.findRelatedTo(request.relatedTo)
        audit["offender_id"] = relatedTo.person.id
        val type = typeProvider(request.type.code)
        val assignation = assignationService.retrieveAssignation(request.assigned, relatedTo.person.manager)
    ...
    private fun Contact.with(outcome: ContactOutcome) = apply {
