...
class AppointmentService internal constructor(
    ...
    private fun <T> UpdatePipeline<T>.validateUpdates(updates: UpdateBuilder<T>) = onEach { (request, existing) ->
        ...
        require(outcome?.outcomeCode != null || amendDateTime == null || amendDateTime.date > LocalDate.now()) {
            "Outcome must be provided when amending an appointment in the past" // TODO TBC with Karen
        }
    }
